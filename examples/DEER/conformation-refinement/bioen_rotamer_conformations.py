...
################################################
#
#   BioEn - bioen_rotamer_conformations.py
#
################################################
...
def check_rotamer_labels(rotamer_labels, nmodel):
    """
    Calculates for each distances between two unpaired electron the
    DEER trace
    """
    for k, v in rotamer_labels.items():
        if v['clash'].count(False) == 0:
            raise ValueError('ERROR: Please consider to remove conformer \'{}\' '.format(nmodel) +
                  'from the list of models. No spin label rotamers could be detected ' +
                  'for spin-label pair \'{}\''.format(k))
    return


def get_deer_from_single_pair_conformations(slp, rotamer_labels, rotamer_library_weights,
    ...
    ndeers = rotamer_labels[l0_ln]['clash'].count(False) * \
            rotamer_labels[l1_ln]['clash'].count(False)
    sim_label = np.zeros((len(slp[2]), ndeers))
    ndeer = 0
    distances = []
        ...
        if not rotamer0_clash[rotamer0.frame]:
            for rotamer1 in rotamer1_site.trajectory:
                if not rotamer1_clash[rotamer1.frame]:
                    ...
                    sim_label[:,ndeer] = ft[:,1]
                    ndeer+=1
    return sim_label.mean(1), distances

# analysis

def get_weighted_rotamer_states(frames_data_lp, bioen_data_lp,
                                label, theta=1000.0):
    """
    ...
    rotamer_weights_2 = dict()
    for r_new, v in rotamer_weights_1.items():
